 ...
 */
function looksNonSecret(name: string, value: string): boolean {
  if (NON_SECRET_NAME_RE.test(name)) return true;
  if (/^https?:\/\//i.test(value)) return true;
  if (/^[A-Z0-9_]+$/.test(value)) return true; // an env-var NAME, not a value
  if (/(test|mock|example|sample|placeholder|dummy|fake|replace|your[-_]|xxxx|\.\.\.)/i.test(value)) return true;
  return false;
}
