...
export function obligationsFor(policy, paths) {
    const out = [];
    for (const def of policy.obligations) {
        const fired = paths.some((p) => def.when.anyPathMatches.some((pattern) => matchesGlob(pattern, p)));
        if (fired)
            out.push({ id: def.id, reason: def.reason, commands: def.run });
    }
    return out;
}
export function parityWarningsFor(policy, paths) {
    const out = [];
    for (const rule of policy.parityRules) {
        if (!paths.some((p) => matchesGlob(rule.match, p)))
            continue;
