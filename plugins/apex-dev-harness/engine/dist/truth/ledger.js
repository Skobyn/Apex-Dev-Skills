...
export function loadLedger(root) {
    ...
    for (const line of raw.split('\n')) {
        ...
        if (cells.length > 0 && /^(surface|status)$/i.test(cells[0]))
            continue; // header row
        ...
        if (routes.length === 0)
            withoutRoutes += 1;
        rows.push({ section, surface, routes, status: m[1], notes: cells[3] ?? '' });
    }
 ...
 */
export function surfaceFor(truth, query) {
