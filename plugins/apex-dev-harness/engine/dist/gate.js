...
export function gate(root, opts = {}) {
    const policy = loadPolicy(root);
    const runner = opts.runner ?? runCommand;
    const warnings = [];
    if (policy.warning)
        warnings.push(policy.warning);
    const paths = opts.paths ?? changedPaths(root, opts.base ?? 'HEAD');
    if (paths.length === 0)
        warnings.push('no changed files detected — the gate ran against an empty diff');
...
function summarizeUnmet(failed, hits) {
    const parts = [];
    if (failed > 0)
        parts.push(`${failed} check${failed === 1 ? '' : 's'} failed`);
    if (hits > 0)
        parts.push(`${hits} watchlist hit${hits === 1 ? '' : 's'}`);
    return parts.join(', ');
}
export function formatGate(v) {
    const lines = [];
    ...
    for (const p of v.parityWarnings)
        lines.push(`parity      ${p} — verify, or state why it diverges.`);
    for (const h of v.watchlistHits)
        lines.push(`watchlist   "${h.term}" (line ${h.line}) — BOUND-006`);
