...
export function prunePolicy(root) {
    ...
    for (const s of SECTIONS) {
        const m = new Map();
        for (const entry of DEFAULT_POLICY[s.name])
            m.set(s.keyOf(entry), entry);
    ...
    for (const s of SECTIONS) {
        ...
        if (!Array.isArray(arr))
            continue; // omitted section — nothing to prune
        const kept = [];
        const removed = [];
        ...
        if (removed.length)
            removedBySection[s.name] = removed;
        if (kept.length)
            keptBySection[s.name] = kept.map((e) => s.keyOf(e));
        out[s.name] = kept;
    }
    ...
    if (Array.isArray(parsed.watchlist)) {
        ...
        const removed = parsed.watchlist.filter((w) => builtinSet.has(w));
        const kept = parsed.watchlist.filter((w) => !builtinSet.has(w));
        if (removed.length) {
            removedBySection.watchlist = removed;
            removedEntries += removed.length;
        }
        if (kept.length)
            keptBySection.watchlist = kept;
        out.watchlist = kept;
    }
