...
export function route(root, query) {
    const lanes = loadLanes(root);
    const ledger = loadLedger(root);
    const policy = loadPolicy(root);
    const warnings = [];
    for (const t of [lanes, ledger, policy])
        if (t.warning)
    ...
    for (const guard of lanes.importGuards) {
        ...
        if (hit) {
            importNotes.push(`${guard.namespace} is LEGACY — this file is a grandfathered importer (${hit.reason}) Do not add new imports.`);
        }
    }
    return {
        query,
...
export function formatRoute(v) {
    ...
    if (v.laneEntry?.replacement)
        lines.push(`${pad('replacement')}${v.laneEntry.replacement}`);
    if (v.surface) {
    ...
    if (v.mwg)
        lines.push(`${pad('mwg')}${v.mwg}`);
    if (v.skills.length)
        lines.push(`${pad('skills')}${v.skills.join(', ')}`);
    if (v.parity.length)
        lines.push(`${pad('parity')}${v.parity.join(' · ')}`);
    for (const n of v.importNotes)
        lines.push(`${pad('imports')}${n}`);
    for (const w of v.warnings)
        lines.push(`${pad('warn')}${w}`);
    return lines.join('\n');
}
