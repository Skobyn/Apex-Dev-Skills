...
- 2026-06-10 — [ADR-0002](0002-tier-routing-via-named-subagents.md) extends the agent surface with three `phase-worker-*` tier subagents (now 4 agents total), adds `tier`/`tokens`/`escalate` to the `apex-scope-loop:outcomes/<slug>/<phase>` sub-key shape, and grows the smoke contract from 10 to 13 checks.
